...
bool AkSoundDataBuilder::DidSwitchContainerDataChange(const TArray<UAkAssetDataSwitchContainerData*>& OldData, const TArray<UAkAssetDataSwitchContainerData*>& NewData)
{
	...
	for (int32 i = 0; i < OldData.Num(); i++)
	{
		...
		if (MediaListsAreDifferent(OldItem->MediaList, (*NewItem)->MediaList))
		{
			return true;
		}
...
bool AkSoundDataBuilder::parsePluginInfo(UAkInitBank* InitBank, const FString& Platform, const TSharedPtr<FJsonObject>& PluginInfoJson)
{
	...
	for (auto& pluginJsonValue : pluginsArrayJson)
	{
		...
		if (pluginDLL == TEXT("DefaultSink") || pluginDLL == TEXT("AkSoundEngineDLL"))
		{
			continue;
		}
...
void AkSoundDataBuilder::cookMediaAsset(const MediaAssetToCook& mediaToCook, const FString& platform)
{
	...
	if (fileReader)
	{
		...
		if (platformData->MediaContentHash != newHash || isStreamedDifferent || prefetchSizeDifferent)
		{
			{
			...
			{
				auto prefetchChunk = new FAkMediaDataChunk(fileReader.Get(), parsedPlatformData->PrefetchSize, bulkDataLoadingType, &mediaAsset->BulkDataWriteLock, true);
				{
					FScopeLock autoLock(&platformData->DataLock);
			...
			fileReader->Seek(0);
			auto dataChunk = new FAkMediaDataChunk(fileReader.Get(), fileSize, parsedPlatformData->IsStreamed ? BULKDATA_Force_NOT_InlinePayload : bulkDataLoadingType, &mediaAsset->BulkDataWriteLock);
			{
				FScopeLock autoLock(&platformData->DataLock);
