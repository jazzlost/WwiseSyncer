...
)
{
	...
	if (!in_pAkEvent->IsMediaReady())
	{
		UE_LOG(LogAkAudio, Warning, TEXT("Not all media for '%s' are loaded. Consider using PostEventAsync()."), *in_pAkEvent->GetName());
	}
...
)
{
	...
	if (AkEvent && !AkEvent->IsMediaReady())
	{
		UE_LOG(LogAkAudio, Warning, TEXT("Not all media for '%s' are loaded. Consider using PostEventAsync()."), *AkEvent->GetName());
	}
	...
	if (PlayingID != AK_INVALID_PLAYING_ID && !bStopWhenAttachedToDestroyed && AkEvent)
	{
	...
	FLatentActionInfo LatentInfo)
{
	...
	if (DeviceAndWorld.IsValid())
	{
		...
		else if (AkEvent && !bStopWhenAttachedToDestroyed)
		{
			AkEvent->PinInGarbageCollector(PlayingID);
		}
	}
